...
class SimulationService:
    def __init__(self, should_publish_to_rabbitmq: bool = True):
        ...
        if self.should_publish_to_rabbitmq:
            ...
            print("Connected to RabbitMQ server in SimulationService")
        
    def set_start_pos(self, q_start: np.ndarray) -> None:
        self.robotarmkinematics_model.set_start_position(q_start)

    def publish_to_rabbitmq(self):
            # Convert values to json serializable types
            self.q_actual = self.robotarmkinematics_model.get_current_position()
            self.qd_actual = self.robotarmkinematics_model.get_current_velocity()
            self.time_stamp = self.step_counter*step_size
            
            msg = {
                RobotArmStateKeys.ROBOT_MODE: self.controller_model.get_state(), # Current mode e.g. RUNNING, IDLE
                RobotArmStateKeys.Q_ACTUAL: self.q_actual.tolist(), # Current joint positions (radians)
                RobotArmStateKeys.QD_ACTUAL: self.qd_actual.tolist(), # Current joint velocities (radians/second)
                RobotArmStateKeys.Q_TARGET: self.controller_model.q_end.tolist(), # Target position (radians)
                RobotArmStateKeys.TIMESTAMP: self.time_stamp, # Time stamp of the status message
                RobotArmStateKeys.JOINT_MAX_SPEED: self.controller_model.get_max_velocity(), # Joint max speed (degrees/second)
                RobotArmStateKeys.JOINT_MAX_ACCELERATION: self.controller_model.get_acceleration(), # Joint max acceleration (degrees/second^2)
                RobotArmStateKeys.TCP_POSE: self.controller_model.get_current_tcp_pose(), # TCP Pose calculation result
            }
    ...
    def load_program(self, q_end: np.ndarray, max_velocity: float, acceleration: float) -> None:
        ...
        q_start: np.ndarray = self.robotarmkinematics_model.get_current_position()
    ...
    def play(self) -> None:
        ...
        while self.step_counter < self.n_steps:
            print(f"Step {self.step_counter}/{self.n_steps}")
            ...
            q_actual: np.ndarray = self.robotarmkinematics_model.get_current_position()
