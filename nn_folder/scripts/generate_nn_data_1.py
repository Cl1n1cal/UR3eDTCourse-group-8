...
for i in range(velocity_count):
    ...
    tmp = max(3, round(random.uniform(10, 70)))
    if tmp > 0.0:
        tmp = np.deg2rad(tmp)
...
for h in range(len(velocities)):
    ...
    for k in range(len(from_pos)):
        ...
        print("step count:", len(traj_q))
        traj_qd = traj_qd.tolist()
        target_q = target_pos[k]
        max_vel = velocities[h]
        max_acc = accelerations[h]
        ...
        for i in range(len(traj_q)):
            ...
            joints["max_vel"] = max_vel
            joints["max_acc"] = max_acc
