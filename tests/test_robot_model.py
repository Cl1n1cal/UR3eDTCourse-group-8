...
model.load_program(end_q, max_velocity=90, acceleration=180)
model.start()
...
for i in range(int(SIM_DURATION / STEP_SIZE)):
    ...
    qd_values.append(model.get_dq_current().copy())
    ddq_values.append(model.get_ddq_current().copy())
